    ...
    $.get("https://my-json-server.typicode.com/vladislavkovtun2003/AJAX/posts", function (data) {
        ...
        for (i = 0; i < data.length; i++) {
            ...
            <tr>
              ...
              <th>`+ data[i].nameInput + `</th>
              <th>`+ data[i].author + `</th>
              <th>`+ data[i].ganre + `</th>
              <th>`+ data[i].date + `</th>
              <th>`+ data[i].sellCount + `</th>
              <th>`+ data[i].price + `</th>
              <th>`+ data[i].dateSell + `</th>
              <th>`+ data[i].receivedCount + `</th>
              <th>`+ data[i].image + `</th>
              <th><a class="delete" href="#" data-id="`+ data[i].id + `">Delete</a>
            </tr>
            `);
        }
    });
...
$(document).ready(function () {
    $("#inputBtn").bind("click", function () {
        $.ajax({
            method: "POST",
            url: "https://my-json-server.typicode.com/vladislavkovtun2003/AJAX/posts",
            data: {
